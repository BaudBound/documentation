...
**A derived field does not resolve:** place it last, after the complete value name, such as `{{items.$length}}`. Every value offers `$length`, `$count`, `$type`, and `$is_empty`, and only as the final part of a reference. A datetime or duration offers its [components](#datetime-and-duration-components) as ordinary path fields with no `﻿---
title: Variables and Data
description: Create, reference, inspect, persist, and safely compare data in BaudBound workflows.
tags: [editor, variables, data]
---
