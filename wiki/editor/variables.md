...
A Script Setting, a declared variable, a Variable Operation value, and a node's Runtime Data all read and write this same ten type vocabulary. A value produced by one can be declared or compared against another without translation.
...
A declared variable or Script Setting declared `float` accepts a whole value such as `300` or `300.0`, and holds it as a float. The type written beside the value is what settles it, the same way a typed language reads `300` into a variable declared as a decimal number.
...
Variable Operation creates and changes named user variables. Variable, Declared Variable, Script Setting, and Secret names use the same portable identifier characters: letters `A-Z` or `a-z`, numbers `0-9`, hyphens, and underscores. No special first character is required. Spaces, dots, slashes, and other characters are rejected by the editor, package schema, and runner.

No prefix is reserved. Every built-in lives behind `@`, which a variable name may not contain, so nothing you name can shadow one and `system_`, `manifest_` and the plain word `settings` are all yours to use. Names are case sensitive and must be unique within the declaration group that owns them.
...
Every declared variable must have an explicit value. String, color, and hotkey defaults cannot be blank, while values such as `false`, `0`, an empty list, and an empty object are valid explicit defaults. The editor changes to match the selected type:
...
Each declaration contains a name, type, description, required option, optional package default, and optional simulation override. Script Settings use the same ten types as declared variables.
...
| Variable | Value |
| --- | --- |
| `{{@manifest.id}}` | Stable script identifier |
| `{{@manifest.name}}` | Script name |
| `{{@manifest.version}}` | Script version |
| `{{@manifest.author}}` | Author |
| `{{@manifest.description}}` | Description |
| `{{@manifest.website}}` | Project website |
| `{{@manifest.source}}` | Source URL |
| `{{@manifest.minimum_runner_version}}` | Minimum compatible runner version |

### System variables

System values are read-only and describe the runner environment at execution time. Most are facts about the machine and are read once when the run starts:

| Variable | Value |
| --- | --- |
| `{{@system.os}}` | Operating system |
| `{{@system.os_name}}` | Full operating system name |
| `{{@system.os_version}}` | Operating system or kernel version |
| `{{@system.arch}}` | CPU architecture |
| `{{@system.hostname}}` | Host name |
| `{{@system.user}}` | Runner user when available |
| `{{@system.locale}}` | Runner locale |
| `{{@system.timezone}}` | Runner time zone |
| `{{@system.cpu_count}}` | Logical CPU count |
| `{{@system.runner_version}}` | Version of the runner executing the script |
| `{{@system.run_id}}` | Unique identifier for this run |
| `{{@system.trigger_id}}` | Node id of the trigger that started this run |
| `{{@system.trigger_type}}` | Action type of that trigger |
| `{{@system.run_started_at}}` | The clock when the run began, as a datetime |

Two are readings rather than facts, and are read afresh for each node execution:

| Variable | Value |
| --- | --- |
| `{{@system.datetime}}` | Current runner date and time, as a datetime |
| `{{@system.uptime}}` | How long the machine has been running, as a duration |

Every reference to a live value inside one node execution reads the same moment, so a single field cannot straddle a tick. The next node reads again, so a loop or a **Delay** sees the clock move. When you need one timestamp for the whole run instead, use `{{@system.run_started_at}}`, which does not move.

`{{@system.datetime}}` is a datetime rather than text, so a script can read one field of it with a [component](#datetime-and-duration-components) or render it with the **Format date and time** operation of [Text Transform](node-reference.md#text-transform).
...
Node outputs use the same ten types as declared variables, Script Settings, and Variable Operations. The Runtime Data panel identifies the exact type produced by each node.
...
### Datetime and duration components

A datetime and a duration each expose their parts as ordinary path fields, so a script can read one without any string handling. These are not `$` names: `$` describes a value, while a component is part of the value.

| Datetime field | Result |
| --- | --- |
| `.full` | The whole moment as RFC 3339 text, offset included |
| `.utc` | The same moment as RFC 3339 text in UTC |
| `.unix` | Seconds since the epoch |
| `.offset_minutes` | Minutes the offset is ahead of UTC |
| `.year` | Calendar year |
...
Duration components are a breakdown, not the same span counted six ways: a duration of 90 seconds reads as `.minutes` `1` and `.seconds` `30`, with `.total_milliseconds` `90000`.

A datetime component is read in the offset the value carries rather than converted to the machine's own zone, so `{{@system.datetime.hour}}` is the hour on the runner's wall clock.

A value's own fields win over a computed one, so a datetime's `.type` and `.value` keep meaning what they always did. Nothing can be read out of a component, because it is computed rather than stored; the only thing that may follow one is a single derived field.

```text
{{@system.datetime.hour}}
{{@system.datetime.weekday}}
{{@system.datetime.full.$length}}
...
A pattern is read in the offset the value carries, matching the [datetime components](#datetime-and-duration-components). Month and weekday names are English.
...
**A derived field does not resolve:** place it last, after the complete value name, such as `{{items.$length}}`. Every value offers `$length`, `$count`, `$type`, and `$is_empty`, and only as the final part of a reference. A datetime or duration offers its [components](#datetime-and-duration-components) as ordinary path fields with no `$`. The older `.$meta.*` form is not supported.
