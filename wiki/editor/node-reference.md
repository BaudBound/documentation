...
- **Configuration:** named **Inputs**, named **Outputs**, and ordered internal **Routes** from an input to one or more outputs. Add, rename, reorder, and remove ports in the properties panel. Each input row lists the outputs it continues to as numbered chips in execution order; pick an output from the row's **Add output** menu to add a route, and use a chip's earlier, later, and remove controls to reorder or drop it. Each output row shows how many inputs reach it, and hovering an output highlights those inputs.
