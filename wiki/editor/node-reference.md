...
- **Configuration:** named **Inputs**, named **Outputs**, and ordered internal **Routes** from an input to one or more outputs. Add, rename, reorder, and remove ports in the properties panel. Routes are edited in a grid with one row per input and one column per output. Click a cell to connect that input to that output, or click it again to disconnect; a connected cell shows the route's execution number within its row, and the earlier and later arrows on a cell reorder it. Each output column shows how many inputs reach it, and a row with no routes is flagged in place.
