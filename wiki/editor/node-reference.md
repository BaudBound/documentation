...
- **Configuration:** named **Inputs**, named **Outputs**, and ordered internal **Routes** from an input to one or more outputs. Add, rename, reorder, and remove ports in the properties panel. The properties panel previews the routing as lines between input pills on the left and output pills on the right; **Edit routes** opens the editor. There, drag from an input's handle onto an output to connect them; each line carries the route's execution number within its input. Click a line or a pill to select it; the toolbar under the canvas then renames, recolors, reorders, or removes the selection. A port's color is shown on its handle on the canvas node, and each editor line fades from its input's color to its output's color. An unrouted input or unreached output is outlined as a warning, and hovering an output shows how many inputs reach it.
