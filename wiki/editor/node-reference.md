...
- **Configuration:** named **Inputs**, named **Outputs**, and ordered internal **Routes** from an input to one or more outputs. Add, rename, reorder, and remove ports in the properties panel. Routes are drawn as lines between input pills on the left and output pills on the right. Drag from an input's handle onto an output to connect them; the line carries the route's execution number within its input. Click a line or a pill to select it; the toolbar under the canvas then renames, reorders, or removes the selection. **Expand** opens the same canvas in a larger dialog for routers with many ports. An unrouted input or unreached output is outlined as a warning, and hovering an output shows how many inputs reach it.
